...
describe.skipIf(!process.env.XKCD_INTEGRATION)("fetchXkcd (integration — real API)", () => {
  it("fetches xkcd #353 (Python) from the real API", async () => {
    const comic = await fetchXkcd(353);

    expect(comic.num).toBe(353);
    expect(comic.title).toBe("Python");
    expect(comic.img).toContain("https://");
    expect(comic.alt).toBeTruthy();
  });

  it("fetches the latest comic from the real API", async () => {
    const comic = await fetchXkcd();

    expect(comic.num).toBeGreaterThan(0);
    expect(comic.title).toBeTruthy();
    expect(comic.img).toContain("https://");
  });

  it("returns 404 for a non-existent comic", async () => {
    await expect(fetchXkcd(999999)).rejects.toThrow(XkcdFetchError);
  });
});
